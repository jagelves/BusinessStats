...
The interval for the population mean when the population standard deviation is unknown is given by $\bar x \pm t_{\alpha/2} \frac {s}{n}$, where $\bar x$ is the point estimate, $t_{a/2} \frac {s}{n}$ is the margin of error, and $\alpha$ is the allowed probability that the interval does not include $\mu$.
...
::: {.blackbox data-latex=""}
1.  The 95% confidence interval for group 1 is \[$-0.36$,$1.86$\].
:::
...
```{r}
(ll1<-mean(sleep$extra[sleep$group==1])+qnorm(0.025)*se1)
(ul1<-mean(sleep$extra[sleep$group==1])-qnorm(0.025)*se1)
```

::: {.blackbox data-latex=""}
2.  The 95% confidence interval for group 2 is \[$1.09$,$3.57$\].
:::
...
```{r}
(ll2<-mean(sleep$extra[sleep$group==2])+qnorm(0.025)*se2)
(ul2<-mean(sleep$extra[sleep$group==2])-qnorm(0.025)*se2)
```
