...
- `=AVERAGE(A1:A9)` → $18.33$
- `=MEDIAN(A1:A9)` → $20$
- `=MODE.MULT(A1:A9)` → $15$ and $20$ (note: use `MODE.MULT` 
  instead of `MODE` to capture all modes — enter it as an array 
...
*To verify in Excel, assuming `PriceCH` is in column D and `PriceMM` 
is in column E, use:*
...
- `=AVERAGE(0.173, 0.196, 0.068, 0.082)` → arithmetic mean
- `=GEOMEAN(1.173, 1.196, 1.068, 1.082)-1` → geometric mean 
- `=1000*(1+GEOMEAN(1.173, 1.196, 1.068, 1.082)-1)^4` → $1621.17$
