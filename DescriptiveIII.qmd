...
quantities rather than relative changes, so no adjustment is needed. 
The geometric mean simplifies to:

::: {.callout-formula}
**THE GEOMETRIC MEAN (Levels)**
$$\bar{x}_g=\sqrt[n]{x_1 \times x_2 \times ... \times x_n}$$
where $x_i$ are positive values such as prices, proportions, or 
index values.
:::
...
::: {.callout-example}
**Example 1:** A company tracks its market share across four regions: 
$x=\{0.20, 0.30, 0.10, 0.10\}$. Since market share is a proportion 
of the whole — a level, not a rate of change — we apply the levels 
formula directly:
...
average. The geometric mean is the appropriate measure here because it accounts for the compounding effect of returns over time.
:::
