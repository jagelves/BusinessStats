...
An **event** is a subset of the sample space. $A$={$4$} is the event of tossing a $4$ when rolling a die.
...
The `gtools` package contains the `combinations()` and `permutations()` functions used to calculate combinations and permutations. Use the *repeats.allowed* argument to specify counting with repetition or no repetition.
...
2.  Let $P(A)=0.4$, $P(B)=0.5$, and $P(A^c \cap B^c)=0.24$. Calculate $P(A^c|B^c)=0.24$, $P(A^c \cup B^c)$, and $P(A \cup B)$.
...
```{r}
table(Crash$`Crash Type`,Crash$`Light Condition`)
```
