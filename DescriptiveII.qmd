...
**4. Add the Relative Frequency:** Next to the frequency column in the pivot table, add a new column labeled "Relative Frequency". In the first cell, enter the formula `=B2/SUM($B$2:$B$5)`, where column B contains the frequencies. The `$` signs lock the denominator so it does not change as you copy the formula down. Copy the formula down for all food items. Each value represents the proportion of students who selected that food item out of the total of 25.
...
1.  Construct a histogram using 7 classes with width 10, starting at 50 (bins: \[50,60), \[60,70), etc.). In which bin is the highest frequency? How many students scored in that range?
